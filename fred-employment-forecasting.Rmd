...
orna = ts(orna[,2], # select the value column
          start = c(2000,1),  # set start of the series January 2000
...
adf.test(diff_ln_orur)
adf.test(diff_orna)
adf.test(diff_orur)
adf.test(diff_cpi)
adf.test(diff_orgdp)
adf.test(diff_ln_orgdp)
```
...
comb = cbind(diff_orna, diff_ln_orur, fedfunds, diff_ln_cpi)
...
# stable model for 2
roots(model2, modulus = TRUE)

varfst = forecast(model2, h = 24)
